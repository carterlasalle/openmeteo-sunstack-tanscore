...
def _request(session, name: str, endpoint: str, params: dict[str, Any], timeout: int = 120) -> FetchResult:
    ...
    try:
        ...
        if isinstance(payload, dict) and payload.get("error"):
            return FetchResult(
                name=name, endpoint=endpoint, params=params, payload=None,
        ...
        return FetchResult(
            name=name, endpoint=endpoint, params=params, payload=None, error=str(exc),
            elapsed_ms=round((time.perf_counter() - started) * 1000.0, 1),
...
def deterministic_params(model: str) -> dict[str, Any]:
    params = {
        **_common(), "forecast_days": config.FORECAST_DAYS, "models": model,
...
def profile_params(model: str) -> dict[str, Any]:
    return {
        **_common(), "forecast_days": min(config.FORECAST_DAYS, 10), "models": model,
...
def hrrr_15min_params() -> dict[str, Any]:
    return {
        **_common(), "models": "ncep_hrrr_conus",
...
def ensemble_params(model: str) -> dict[str, Any]:
    return {
        **_common(), "forecast_days": config.FORECAST_DAYS, "temporal_resolution": "hourly",
        "models": model, "hourly": ",".join(config.ENSEMBLE_VARIABLES),
    }
...
def air_quality_params() -> dict[str, Any]:
    return {
        ...
        "timezone": config.TIMEZONE, "forecast_days": config.AIR_QUALITY_DAYS,
        "domains": "cams_global", "hourly": ",".join(config.AIR_QUALITY_VARIABLES),
    }
...
def fetch_all(cache_dir: Path, fresh: bool = True) -> list[FetchResult]:
    ...
    for model in config.DETERMINISTIC_MODELS:
        results.append(_request(session, f"deterministic__{model}", FORECAST_URL, deterministic_params(model)))
    for model in config.PROFILE_MODELS:
        results.append(_request(session, f"profile__{model}", FORECAST_URL, profile_params(model)))
    results.append(_request(session, "hrrr_15min", FORECAST_URL, hrrr_15min_params()))
    for model in config.ENSEMBLE_MEMBER_MODELS:
        results.append(_request(session, f"ensemble_members__{model}", ENSEMBLE_URL, ensemble_params(model)))
    for model in config.ENSEMBLE_MEAN_MODELS:
        results.append(_request(session, f"ensemble_mean__{model}", ENSEMBLE_URL, ensemble_mean_params(model)))
    results.append(_request(session, "air_quality", AIR_QUALITY_URL, air_quality_params()))
    return results
...
def probe_live(timeout: int = 20) -> list[FetchResult]:
    ...
    probes = [
        ("forecast__best_match", FORECAST_URL, {**_common(), "forecast_days": 1, "models": "best_match", "hourly": "temperature_2m"}),
        ("ensemble__ncep_gefs025", ENSEMBLE_URL, {**_common(), "forecast_days": 1, "models": "ncep_gefs025", "hourly": "temperature_2m"}),
...
def write_raw(results: list[FetchResult], raw_dir: Path) -> None:
    ...
    for result in results:
        entry = {
            "name": result.name, "endpoint": result.endpoint, "params": result.params,
