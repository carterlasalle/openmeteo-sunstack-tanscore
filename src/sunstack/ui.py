...
<div class="controls"><label>Location <select id="locsel"><option value="">Loading…</option></select></label><label>Skin <select id="skin"><option value="">None</option><option value="1">I</option><option value="2">II</option><option value="3">III</option><option value="4">IV</option><option value="5">V</option><option value="6">VI</option></select></label><label style="white-space:nowrap" title="Only if you have a measured or estimated personal MMD"><input id="mmd_on" type="checkbox" onchange="var w=document.getElementById('mmdwrap');w.style.display=this.checked?'':'none';if(!this.checked){document.getElementById('mmd').value='';document.getElementById('mmdbasis').value='';}"> I have my MMD</label><span id="mmdwrap" style="display:none"><label>My MMD <input id="mmd" type="number" min="1" step="1" placeholder="J/m²" title="Measured/estimated personal MMD in melanogenic-effective J/m² — leave blank unless you know yours" style="width:80px"></label><label>basis <select id="mmdbasis" title="Where the MMD number came from: MEASURED (phototesting), OBJECTIVE_ESTIMATE (instrument data), COARSE_ESTIMATE (rough traits, wide uncertainty)"><option value="">—</option><option value="MEASURED">MEASURED</option><option value="OBJECTIVE_ESTIMATE">OBJECTIVE_ESTIMATE</option><option value="COARSE_ESTIMATE">COARSE_ESTIMATE</option></select></label></span>
...
<div class="daydetail" id="charts"><h3>Day charts (separate panels — SED is exposure, never “good”)</h3><canvas id="chartScore" width="640" height="150" style="width:100%;border:1px solid var(--line);border-radius:8px;background:#fffdf7"></canvas><canvas id="chartDose" width="640" height="120" style="width:100%;border:1px solid var(--line);border-radius:8px;background:#fffdf7;margin-top:8px"></canvas><canvas id="chartSed" width="640" height="120" style="width:100%;border:1px solid var(--line);border-radius:8px;background:#fffdf7;margin-top:8px"></canvas><div class="note">TanScore fixed 0–100 (top, instantaneous intensity); cumulative TanDose melanogenic J/m² (middle, exposure odometer); cumulative SED (bottom, sunburn-weighted exposure, never “good”). Shaded band = best window. Pigment-darkening lives in tables under Advanced, never merged into TanScore.</div></div>
...
function disagreeNote(x){return [x.outdoor_block_reason,x.uv_input_disagree?'UV/broadband inputs disagree on cloud':'',x.uvi_source_disagree?'OM/CAMS UVI disagree':''].filter(Boolean).join(' · ');}
let FIG=sunFigState();
...
<h3>Every hour — UVI consensus first (OM = Open-Meteo/GFS, CAMS = spectral, EPA = operational)</h3><div class="tablewrap"><table><thead><tr><th>Time</th><th>UV∘</th><th>OM</th><th>CAMS</th><th>EPA</th><th>Clear</th><th>ΔUV</th><th>UVA</th><th>UVB</th><th>Temp</th><th>Wind</th><th>Cloud</th><th>Rain</th><th>DNI</th><th>Overall</th><th>Abs</th><th>Local</th><th>Atm</th><th>Conf</th><th>Note</th></tr></thead><tbody>${hHtml||'<tr><td colspan="19">No hourly rows for this day.</td></tr>'}</tbody></table></div>
