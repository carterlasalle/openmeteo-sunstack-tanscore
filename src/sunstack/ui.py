...
function stickFigure(x,y,s,liftDeg,faceDir){
 // Side-view recliner silhouette (filled body, like the reference outline
 // tipped back): hip pivot at (x,y) ON the ground line. One thick capsule
 // for the legs stretched flat along the ground, one thick capsule for the
 // torso rising toward the facing side at liftDeg above horizontal, solid
 // disc head atop the torso, tiny foot tick. faceDir=+1 faces right.
 const dir=faceDir<0?-1:1;
 const lift=(liftDeg==null?0:liftDeg)*Math.PI/180;
 const legL=s*1.0,torL=s*0.62,headR=s*0.125;
 const ankX=x-dir*legL,ankY=y;
 const shX=x+dir*torL*Math.cos(lift),shY=y-torL*Math.sin(lift);
 const hdX=shX+dir*(headR+2)*Math.cos(lift),hdY=shY-(headR+2)*Math.sin(lift);
 let o='';
 o+=`<ellipse cx="${(x-dir*legL*0.42).toFixed(1)}" cy="${(y+6).toFixed(1)}" rx="${(legL*0.55).toFixed(1)}" ry="2.5" fill="currentColor" opacity="0.12"/>`;
 o+=`<line x1="${x}" y1="${y}" x2="${ankX.toFixed(1)}" y2="${ankY}" stroke="currentColor" stroke-width="10" stroke-linecap="round"/>`;
 o+=`<line x1="${ankX.toFixed(1)}" y1="${ankY}" x2="${(ankX-dir*2).toFixed(1)}" y2="${(ankY-7).toFixed(1)}" stroke="currentColor" stroke-width="7" stroke-linecap="round"/>`;
 o+=`<line x1="${x}" y1="${(y-1).toFixed(1)}" x2="${shX.toFixed(1)}" y2="${shY.toFixed(1)}" stroke="currentColor" stroke-width="17" stroke-linecap="round"/>`;
 o+=`<circle cx="${hdX.toFixed(1)}" cy="${hdY.toFixed(1)}" r="${headR.toFixed(1)}" fill="currentColor"/>`;
 return o;}
function drawSunFig(row){const svg=document.getElementById('sunfig');if(!svg||!row)return;const cx=150,cy=152,r=100;const NS='http://www.w3.org/2000/svg';while(svg.firstChild)svg.removeChild(svg.firstChild);const mk=(t,a)=>{const e=document.createElementNS(NS,t);for(const k in a)e.setAttribute(k,a[k]);svg.appendChild(e);return e;};mk('line',{x1:16,y1:cy,x2:284,y2:cy,stroke:'#c9bfa8','stroke-width':1.5});mk('text',{x:248,y:cy-14,'font-size':10,fill:'#6f6553'}).textContent='ground';const elev=+row.solar_elevation_deg,az=+row.solar_azimuth_deg;const lift=+(row.torso_lift_deg||0);if(!(elev>0)){mk('text',{x:cx,y:64,'text-anchor':'middle','font-size':13,fill:'#6f6553'}).textContent='sun below horizon';mk('text',{x:cx,y:82,'text-anchor':'middle','font-size':11,fill:'#6f6553'}).textContent='no direct-sun posture';const g=document.createElementNS(NS,'g');g.setAttribute('transform',`translate(${cx+8},${cy-4})`);g.setAttribute('color','#9a938a');g.innerHTML=stickFigure(0,0,64,0,1);svg.appendChild(g);return;}const cdeg=compassDeg(row.sun_compass);const azUse=cdeg==null?(Number.isNaN(az)?180:az):cdeg;const[sx,sy]=figXY(cx,cy-4,r,elev,azUse);const sun=mk('circle',{cx:sx,cy:sy,r:11,fill:'#e8a100',stroke:'#b25e00','stroke-width':2});sun.appendChild(document.createElementNS(NS,'title')).textContent=`sun ${elev.toFixed(0)}° up, ${row.sun_compass||''}`;const dir=sx>=cx?1:-1;const g=document.createElementNS(NS,'g');g.setAttribute('transform',`translate(${cx},${cy-4})`);g.setAttribute('color','#211c12');g.innerHTML=stickFigure(0,0,64,lift,dir);svg.appendChild(g);const lr=lift*Math.PI/180,tL=64*0.62,chX=cx+dir*tL*0.55*Math.cos(lr),chY=(cy-4)-tL*0.55*Math.sin(lr);mk('line',{x1:chX.toFixed(1),y1:chY.toFixed(1),x2:sx.toFixed(1),y2:sy.toFixed(1),stroke:'#b25e00','stroke-width':1.2,'stroke-dasharray':'4 3',opacity:0.7});mk('text',{x:cx,y:cy+22,'text-anchor':'middle','font-size':10,fill:'#6f6553'}).textContent=`face ${row.sun_compass||'—'} · torso ~${Math.round(lift)}°`;const cap=document.getElementById('suncap');if(cap)cap.innerHTML=`<b>${hhmm(row.time)}</b> — ${esc(row.sun_posture_guidance||'')} (UV ${f1(row.uv_index)}, overall ${f0(row.overall_tan_opportunity_0_100)})`;}
