...
function stickFigure(x,y,s,liftDeg,faceDir){const legL=s*0.55,torL=s*0.5,headR=s*0.14,armL=s*0.38;const lift=(liftDeg==null?0:liftDeg)*Math.PI/180;const hx=x,hy=y-legL-torL*Math.cos(lift);const tx=x+torL*Math.sin(lift)*(faceDir<0?-1:1);const ty=y-legL-torL*Math.cos(lift);let o='';o+=`<line x1="${x}" y1="${y}" x2="${x}" y2="${(y-legL).toFixed(1)}" stroke="currentColor" stroke-width="3" stroke-linecap="round"/>`;o+=`<line x1="${x}" y1="${(y-legL).toFixed(1)}" x2="${tx.toFixed(1)}" y2="${ty.toFixed(1)}" stroke="currentColor" stroke-width="3" stroke-linecap="round"/>`;o+=`<line x1="${x}" y1="${(y-legL*0.55).toFixed(1)}" x2="${(x-s*0.42).toFixed(1)}" y2="${(y-legL*0.55+s*0.3).toFixed(1)}" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/>`;o+=`<line x1="${x}" y1="${(y-legL*0.55).toFixed(1)}" x2="${(x+s*0.42).toFixed(1)}" y2="${(y-legL*0.55+s*0.3).toFixed(1)}" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/>`;const ax=(hx+tx)/2,ay=(hy+ty)/2;const anx=ax+armL*Math.sin(lift)*(faceDir<0?-1:1),any=ay-armL*Math.cos(lift);o+=`<line x1="${ax.toFixed(1)}" y1="${ay.toFixed(1)}" x2="${anx.toFixed(1)}" y2="${any.toFixed(1)}" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/>`;o+=`<circle cx="${hx.toFixed(1)}" cy="${(hy-headR-2).toFixed(1)}" r="${headR}" fill="none" stroke="currentColor" stroke-width="2.4"/>`;return o;}
function drawSunFig(row){const svg=document.getElementById('sunfig');if(!svg||!row)return;const W=300,H=190,cx=150,cy=158,r=118;const NS='http://www.w3.org/2000/svg';while(svg.firstChild)svg.removeChild(svg.firstChild);const mk=(t,a)=>{const e=document.createElementNS(NS,t);for(const k in a)e.setAttribute(k,a[k]);svg.appendChild(e);return e;};mk('line',{x1:20,y1:cy,x2:280,y2:cy,stroke:'#c9bfa8','stroke-width':1.5});mk('text',{x:24,y:cy-6,'font-size':10,fill:'#6f6553'}).textContent='horizon — legs flat, parallel to ground';const elev=+row.solar_elevation_deg,az=+row.solar_azimuth_deg;if(!(elev>0)){mk('text',{x:cx,y:70,'text-anchor':'middle','font-size':13,fill:'#6f6553'}).textContent='sun below horizon';mk('text',{x:cx,y:88,'text-anchor':'middle','font-size':11,fill:'#6f6553'}).textContent='no direct-sun posture';const g=document.createElementNS(NS,'g');g.setAttribute('transform',`translate(${cx-60},${cy-86})`);g.setAttribute('color','#9a938a');g.innerHTML=stickFigure(60,80,52,0,1);svg.appendChild(g);return;}const cdeg=compassDeg(row.sun_compass);const azUse=cdeg==null?(Number.isNaN(az)?180:az):cdeg;const[ sx,sy]=figXY(cx,cy,r,elev,azUse);const sun=mk('circle',{cx:sx,cy:sy,r:11,fill:'#e8a100',stroke:'#b25e00','stroke-width':2});sun.appendChild(document.createElementNS(NS,'title')).textContent=`sun ${elev.toFixed(0)}° up, ${row.sun_compass||''}`;mk('line',{x1:cx,y1:cy,x2:sx,y2:sy,stroke:'#b25e00','stroke-width':1.2,'stroke-dasharray':'4 3',opacity:0.7});const fx=cx+(azUse>180?-46:46);const g=document.createElementNS(NS,'g');g.setAttribute('transform',`translate(${fx-60},${cy-86})`);g.setAttribute('color','#211c12');g.innerHTML=stickFigure(60,80,52,+(row.torso_lift_deg||0),azUse>180?-1:1);svg.appendChild(g);mk('text',{x:cx,y:cy+16,'text-anchor':'middle','font-size':10,fill:'#6f6553'}).textContent=`face ${row.sun_compass||'—'} · torso ~${Math.round(+(row.torso_lift_deg||0))}°`;const cap=document.getElementById('suncap');if(cap)cap.innerHTML=`<b>${hhmm(row.time)}</b> — ${esc(row.sun_posture_guidance||'')} (UV ${f1(row.uv_index)}, overall ${f0(row.overall_tan_opportunity_0_100)})`;}
